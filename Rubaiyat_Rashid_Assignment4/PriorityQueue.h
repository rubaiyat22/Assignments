...
public:
	...
	bool Find(const Comparable & x)
	{
		...
		//if (node == nullptr)
			//return false;
		if (hash_table_.Contains(x) && node -> element_)
			return true;

		return false;

			
		//return hash_table_.Contains(x) &&  hash_table_.GetElement(x) -> element_ == x && hash_table_.GetElement(x) != nullptr ;
	}    
...
private:
	...
	HashTable<Comparable, BinomialNode*> hash_table_;         // A hashtable containing all the element_s in the queue
	...
	BinomialNode * CombineTrees( BinomialNode *t1, BinomialNode *t2 )
	{
		...
		t2 -> next_sibiling_ = t1 -> left_child_; //given
		t2 -> parent_ = t1;  //t1 becomes the parent of t2
		t1->left_child_ = t2; //given
		return t1;
	}
	...
	void PercolateUp (BinomialNode* &t)
	{
		...
		{
			//hash_table_.Remove(t -> parent_ -> element_);
	...
	void NewMerge(BinomialNode * a_element)
	{
		...
		for( int i = 0; i < current_size_; ++i)
		{
			...
			carry = CombineTrees(the_trees_[i], carry);
			the_trees_[i] = nullptr;
		}
	}
};
